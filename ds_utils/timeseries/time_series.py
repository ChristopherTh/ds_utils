...
class ts_eda:
	...
	def polar_plot(self, ):
		...
		for  year in [2012, 2020]:
			times = pd.date_range(self.ts_df[self.ts_df.index.year == year].index.min().to_pydatetime(), self.ts_df[self.ts_df.index.year == year].index.max().to_pydatetime())
	...
	def lag_plot(self,):
		...
		for i, axis in enumerate(axs.flat):
			sns.scatterplot(x = f'lag_{i + 1 }', y = 0, data = self.ts_df, ax = axis, hue = 'month')
